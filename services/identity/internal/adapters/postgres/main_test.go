...
// (LC-002 / LC-014) so the identity integration + RLS tests run under
...
//     ledgercore_identity_rt (runtime, NOBYPASSRLS, DML only) idempotently;
//   * (re)creates schema `identity` owned by the migrator with the runtime
...
const (
	...
	identityRTPassword = "ledgercore_identity_rt_dev"
	schemaName         = "identity"
	runtimeRole        = "ledgercore_identity_rt"
)
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'ledgercore_identity_rt') THEN
        CREATE ROLE ledgercore_identity_rt LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOBYPASSRLS;
    END IF;
END $$;`
	...
	for _, stmt := range []string{
		...
		fmt.Sprintf("ALTER ROLE ledgercore_identity_rt LOGIN PASSWORD %s", quoteLiteral(identityRTPassword)),
		"GRANT ledgercore_maint TO ledgercore_migrator",
		// Fresh schema owned by the migrator — guarantees clean ownership even
		// if an older run left an `identity` schema owned by another role.
		"DROP SCHEMA IF EXISTS identity CASCADE",
		"CREATE SCHEMA identity AUTHORIZATION ledgercore_migrator",
		"GRANT USAGE ON SCHEMA identity TO ledgercore_identity_rt",
		"ALTER DEFAULT PRIVILEGES FOR ROLE ledgercore_migrator IN SCHEMA identity GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ledgercore_identity_rt",
		"ALTER DEFAULT PRIVILEGES FOR ROLE ledgercore_migrator IN SCHEMA identity GRANT USAGE, SELECT ON SEQUENCES TO ledgercore_identity_rt",
	} {
	...
	// 3. Apply the grants.sql step (as admin/superuser): the maint role needs
	//    USAGE + SELECT/DELETE on the schema's tables, and every SECURITY DEFINER
	//    function is reassigned to ledgercore_maint with EXECUTE to the runtime
	//    role. Generic on purpose (mirrors infra/postgres/migrate/grants.sql).
	for _, stmt := range []string{
		"GRANT USAGE ON SCHEMA identity TO ledgercore_maint",
		"GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA identity TO ledgercore_identity_rt",
		"GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA identity TO ledgercore_identity_rt",
		"GRANT SELECT, DELETE ON ALL TABLES IN SCHEMA identity TO ledgercore_maint",
	} {
		if _, err := admin.Exec(ctx, stmt); err != nil {
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format('GRANT EXECUTE ON FUNCTION %s TO ledgercore_identity_rt', fn);
    END LOOP;
END $$;`
	...
	runtimeURL, err := withUser(adminURL, runtimeRole, identityRTPassword)
	if err != nil {
		return err
	}
